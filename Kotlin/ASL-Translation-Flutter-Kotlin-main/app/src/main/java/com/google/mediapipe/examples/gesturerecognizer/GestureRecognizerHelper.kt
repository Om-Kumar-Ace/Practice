...
class GestureRecognizerHelper(
    ...
    fun recognizeLiveStream(
        ...
        //Using manual processing of imageproxy to avoid conflicts between the two helpers/models
    ...
    //This function is disabled for our release. It is a method to allow recognition of video files.
    ...
    private fun returnLivestreamResult(
        ...
        //Send live hand gestures to multi model for word recognition.
        val recognizedGestureResults = result.gestures()

        if (recognizedGestureResults.isNotEmpty() && recognizedGestureResults[0].isNotEmpty()) {
            //Send result to multiModel helper in realtime.
            val resultString = recognizedGestureResults[0].toString()
            val gestureLabel = resultString.substringAfter("<Category \"").substringBefore("\"")
            multiModelHelper.updateGestureLabel(gestureLabel)
        } else {
            Log.d("GestureRecognizerLog", "No gestures recognized or resultLabel is empty")
        }
    ...
    //Defining our companion object parameters and the model task file url.
    companion object {
